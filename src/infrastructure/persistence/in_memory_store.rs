...
use super::{
    string_store::StringStore, list_store::ListStore,
    zset_store::ZSetStore, stream_store::StreamStore,
};
...
impl StorePort for InMemoryStore {
    fn get(&self, key: &str) -> Option<String> { self.strings.get(key) }
    ...
    fn rpush(&self, key: &str, values: Vec<String>) -> i64 {
        let v = self.lists.rpush(key, values); self.bump_version(key); v
    }

    fn lpush(&self, key: &str, values: Vec<String>) -> i64 {
        let v = self.lists.lpush(key, values); self.bump_version(key); v
    }

    fn lpop(&self, key: &str) -> Option<String> {
        let v = self.lists.lpop(key)?; self.bump_version(key); Some(v)
    }

    fn lpop_count(&self, key: &str, count: usize) -> Vec<String> {
        let v = self.lists.lpop_count(key, count);
        if !v.is_empty() { self.bump_version(key); }
        v
    }

    fn rpop(&self, key: &str) -> Option<String> {
        let v = self.lists.rpop(key)?; self.bump_version(key); Some(v)
    }
    ...
    fn lrem(&self, key: &str, count: i64, value: &str) -> i64 {
        let v = self.lists.lrem(key, count, value);
        if v > 0 { self.bump_version(key); }
        v
    }

    fn llen(&self, key: &str) -> i64 { self.lists.llen(key) }

    fn zadd(&self, key: &str, pairs: Vec<(f64, String)>) -> i64 {
        let v = self.zsets.zadd(key, pairs); self.bump_version(key); v
    }
    ...
    fn zrem(&self, key: &str, members: &[String]) -> i64 {
        let v = self.zsets.zrem(key, members);
        if v > 0 { self.bump_version(key); }
        v
    }

    fn geoadd(&self, key: &str, lon: f64, lat: f64, member: String) -> bool {
        let v = self.zsets.geoadd(key, lon, lat, member); self.bump_version(key); v
    }
    ...
    fn xadd(&self, key: &str, id: &str, fields: Vec<(String, String)>) -> Option<String> {
        let v = self.streams.xadd(key, id, fields)?; self.bump_version(key); Some(v)
    }
