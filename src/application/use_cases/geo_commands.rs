...
pub struct GeoCommands<S: StorePort> {
    store: S,
}

impl<S: StorePort> GeoCommands<S> {
    ...
    pub fn geosearch(&self, args: &[String]) -> Bytes {
        ...
        let radius_m = GeoUtils::to_metres(radius, unit);
        i += 3;
        ...
        let mut hits = self
            .store
            .geosearch_radius(key, center_lon, center_lat, radius_m);
        ...
        RespEncoder::array(
            hits.into_iter()
                .map(|(m, _)| RespEncoder::bulk_string(&m))
                .collect(),
        )
    }
}
